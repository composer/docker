...
build:
	docker build -t composer:1.6 -t composer:1.6.5 -t composer:1 -t composer:latest 1.6
	docker build -t composer:1.5 -t composer:1.5.6 1.5
	docker build -t composer:1.4 -t composer:1.4.3 1.4

test:
	docker run -t composer:latest --no-ansi | grep 'Composer version 1.6.5'
	docker run -t composer:1 --no-ansi | grep 'Composer version 1.6.5'
