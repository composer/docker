...
build:
	...
	docker build -t composer:1.4 -t composer:1.4.0 -t composer:1 -t composer:latest 1.4
...
test:
	...
	docker run -t composer:1.4 --no-ansi | grep 'Composer version 1.4.0'
	...
	docker run -t composer:1 --no-ansi | grep 'Composer version 1.4.0'
	docker run -t composer:1 composer --version --no-ansi | grep 'Composer version 1.4.0'
	docker run -t composer:1 /bin/bash --version
	docker run -t composer:latest --no-ansi | grep 'Composer version 1.4.0'
