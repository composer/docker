...
build:
	docker build -t composer:1.5 -t composer:1.5.0 -t composer:1 -t composer:latest 1.5
...
test:
	docker run -t composer:latest --no-ansi | grep 'Composer version 1.5.0'
	docker run -t composer:latest /bin/bash --version
	docker run -t composer:1 --no-ansi | grep 'Composer version 1.5.0'
	docker run -t composer:1 /bin/bash --version
	docker run -t composer:1.5 --no-ansi | grep 'Composer version 1.5.0'
