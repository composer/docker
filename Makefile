...
build:
	docker build -t composer:1.7 -t composer:1.7.1 -t composer:1 -t composer:latest 1.7
...
test:
	docker run -t composer:latest --no-ansi | grep 'Composer version 1.7.1'
	docker run -t composer:1 --no-ansi | grep 'Composer version 1.7.1'
